...
Eb = 0;
N0 = Eb * 10.^(-EbN0dB/10);  % Densidade espectral do ruído
...
for ik = 1:length(EbN0dB)
    % Adição de ruído
    noise = sqrt(N0)*randn(1,N)+1i*sqrt(N0)*randn(1,N); % Influência no eixo imaginário e real, isto é, influencia na amplitude e na fase do sinal
    noise_bpsk = sqrt(NO)*randn(1,N)+1i*sqrt(N0)*randn(1,N); % Influência no eixo imaginário e real, isto é, influencia na amplitude e na fase do sinal
    %
