...
% Vetor dos raios, começa em 1000, incrementado a cada 10 e termina em 12000.
vtdR = [1000:10:12000]; 
...
for iFc = 1:length(vtFc) %for para percorrer o vetor das frequência, vai ser feito 5x.
    ...
    for idR = 1:length(vtdR) % For para percorrer o vetor dos raios (vtdR).
        ...
        dIntersiteDistance = 2*sqrt(3/4)*dR;  % Distância entre ERBs
        ...
        vtBs = vtBs + (dDimX/2 + j*dDimY/2); %Ajuste de posiï¿½ï¿½o das bases (posiï¿½ï¿½o relativa ao canto inferior esquerdo).
        ...
        for iBsD = 1:length(vtBs)  % Loop nas 7 ERBs.
            % Matriz 3D com os pontos de medição de cada ERB. Os pontos são modelados como némeros complexos X +jY, sendo X a posição na abcissa e Y, a posição no eixo das ordenadas.
            ...
            mtDistEachBs = abs(mtPosEachBS);   % Distância entre cada ponto de medido e a sua ERB.
            ...
            mtPowerEachBSdBm = dPtdBm - mtPldB;  % Potências recebidas em cada ponto de medido.
            %Cálculo da maior potência em cada ponto de medido.
        ...
        if (dOutRate <= 10)
            ...
            vtdRdOutRate(idR) = dR;
        elseif (dOutRate > 10) % caso o raio seja menor que 10, deve ser preenchido por zero para a posição em questão.
            vtdOutRate(idR) = 0;
            vtdRdOutRate(idR) = 0;
        end  
